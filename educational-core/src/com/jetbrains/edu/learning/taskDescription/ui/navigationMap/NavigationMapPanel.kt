package com.jetbrains.edu.learning.taskDescription.ui.navigationMap
...
class NavigationMapPanel : JPanel(BorderLayout()) {
  ...
  init {
    ...
    toolbar.component.border = JBEmptyBorder(6, 0, 16, 0)
...
class NavigationMapAction(val task: Task, private val currentTask: Task) : DumbAwareAction() {
  ...
  override fun update(e: AnActionEvent) {
    ...
    if ((task.index > 1) && NavigationUtils.isUnsolvedHyperskillStage(task.lesson.taskList[task.index - 2])) presentation.isEnabled = false
