...
object AdditionalFilesUtils {
  ...
  fun inCourseIgnore(file: VirtualFile, project: Project): Boolean {
    ...
    val courseIgnoreChecker = CourseIgnoreChecker.getInstance(project)
    return courseIgnoreChecker.isIgnored(courseRelativePathFixedForDirectory)
  }

  private fun isExcluded(file: VirtualFile, course: Course, project: Project): Boolean =
    inCourseIgnore(file, project) || excludedByConfigurator(file, course, project)
  ...
  private fun additionalFilesVisitor(project: Project, course: Course) =
    object : VirtualFileVisitor<Any>(NO_FOLLOW_SYMLINKS) {
      ...
      override fun visitFile(file: VirtualFile): Boolean {
        if (FileUtil.toSystemDependentName(file.path) == archiveLocation || isExcluded(file, course, project)) {
          return false
        }
