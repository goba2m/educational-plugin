...
| `GradleConfiguratorBase`              | `settings.gradle` excluded only if it is not Hyperskill and if it is the same as default<br/>`out/`<br/>`build/`<br/>`gradle/`<br/>`EduTestRunner.java`<br/>`gradlew`<br/>`gradlew.bat`<br/>`local.properties`<br/>`settings.gradle`<br/>`gradle-wrapper.jar`<br/>`gradle-wrapper.properties`                                                                                                                                                                                                  |
...
For example, the line `a.txt` in version 0 means only a file
in the course root directory.
In version 1 this means any file named `a.txt`.

The set of implicitly ignored files is the same as in version 0.
...
#### Version 2
This version is not implemented, but it is expected to reduce the set of implicitly ignored files to
a minimal amount such as only the `.idea` directory, task descriptions and course configuration files. 
